...
public class PageIndex {
	public static String pageList(int page,int totpage,String url,String addtag,int boardid) {
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	// Prev 표시 부분
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+	"&boardid="+ boardid+addtag+"'>"
	  	...
	  	for(int i=0;i<10;i++,idx_start++) {
	  		...
	  		if(idx_start == page)
	  			...
	  			s_idx = s_idx + " <a href='" + url + "?page="  + idx_start+"&boardid="+ boardid;
	  			s_idx = s_idx + addtag + "'> " + idx_start + " </a> ";
	  		}
	  	}
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a href='"+url+"?page="+idx_start+"&boardid="+boardid+addtag+"'>"
	...
	public static String pageListHan(int page,int totpage,String url,String query, String key,int boardid) {
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	// Prev 표시 부분
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+"&boardid="+ boardid +"&search="+query+"&key="+ URLEncoder.encode(key)+"'>"
		...
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a href='"+url+"?page="+idx_start+"&boardid="+ boardid + "&search="+query+"&key="+ URLEncoder.encode(key)+"'>"
